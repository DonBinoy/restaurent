...
export default function ProductDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-white font-sans text-gray-900">
      ...
      <div className="max-w-[1200px] mx-auto px-6 pb-24 w-full flex flex-col md:flex-row gap-12 lg:gap-20">
        
        {/* LEFT COLUMN: Image Gallery */}
        <div className="w-full md:w-1/2 flex flex-col gap-4 sticky top-28 h-max">
          {/* Main Image */}
          <div className="w-full aspect-square bg-[#F9F9F9] rounded-2xl flex items-center justify-center p-8 border border-gray-100 relative group">
            <Image 
              src={product.image} 
              alt={product.name} 
              fill 
              className="object-contain drop-shadow-xl p-12 transition-transform duration-500 group-hover:scale-105"
              priority
            />
          </div>
          {/* Thumbnails */}
          <div className="flex gap-4">
            {[product.image, product.image, product.image].map((img, i) => (
              <div key={i} className={`w-20 h-20 rounded-xl relative border-2 flex items-center justify-center p-2 cursor-pointer transition-colors ${i === 0 ? 'border-[var(--color-primary)] bg-white' : 'border-transparent bg-[#F9F9F9] hover:border-gray-300'}`}>
                 <Image src={img} alt={`Thumbnail ${i+1}`} fill className="object-contain p-3 drop-shadow-md" />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="w-full md:w-1/2 flex flex-col">
          ...
          <h1 className="font-serif font-bold text-4xl lg:text-5xl text-gray-900 leading-tight mb-2">
            {product.name}
          </h1>
            ...
            <div className="flex gap-3">
              {['250g', '500g', '1kg'].map(w => (
                <button 
                  ...
                  className={`px-6 py-3 rounded-lg border text-sm font-bold transition-all ${selectedWeight === w ? 'border-[var(--color-primary)] bg-[var(--color-primary)] text-white' : 'border-gray-200 text-gray-600 hover:border-gray-400'}`}
                >
                  {w}
                </button>
              ))}
          ...
          <div className="flex flex-col gap-4 mb-10">
            <div className="flex items-center gap-4">
              <div className="flex items-center border border-gray-300 rounded-lg h-14 px-2 w-32">
                <button onClick={() => setQty(Math.max(1, qty - 1))} className="flex-1 flex items-center justify-center text-gray-600 hover:text-black"><Minus size={18} /></button>
                <span className="w-8 text-center font-bold">{qty}</span>
                <button onClick={() => setQty(qty + 1)} className="flex-1 flex items-center justify-center text-gray-600 hover:text-black"><Plus size={18} /></button>
              </div>
              
              <button className="flex-1 bg-white border-2 border-[var(--color-primary)] text-[var(--color-primary)] h-14 rounded-lg font-bold text-sm uppercase tracking-widest hover:bg-gray-50 transition-colors flex items-center justify-center gap-2">
                <ShoppingBag size={18} /> Add To Cart
              </button>
            </div>
            
            <button className="w-full bg-[var(--color-primary)] text-white h-14 rounded-lg font-bold text-sm uppercase tracking-widest hover:bg-[#031d3a] transition-colors shadow-lg">
              Buy It Now
            </button>
          </div>
